    ...
    for person in os.listdir(root):
        ...
        for file in os.listdir(person_dir):
            if file.lower().endswith(".pgm"):
                image_paths.append(os.path.join(person_dir, file))
...
def main(param):
    ...
    face_model=QualityModel(param.model_path,None, param.gpu_id, param.backbone)
    for dataset in datasets:
        ...
        image_list = load_all_images(root)#
        absolute_list = image_list#
        ...
        save_path = os.path.join(param.data_dir, param.score_file_name)

        with open(save_path, "w") as f:
            for i in range(len(quality)):
                f.write(image_list[i] + " " + str(quality[i][0]) + "\n")
